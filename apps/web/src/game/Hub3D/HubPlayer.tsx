...
function readGroupMotion(group: { position: { x: number; z: number } }, motion: MotionState): { isMoving: boolean; dx: number } {
  ...
  if (Math.abs(dx) > FACING_EPSILON) {
    motion.facingRef.current = dx > 0 ? 1 : -1;
  }
  return { isMoving: dx * dx + dz * dz > MOVE_EPSILON_SQ, dx };
}
...
function AnimatedSpritePawn({ skin }: { skin: SkinConfig }): ReactElement {
  ...
  useFrame((_, delta): void => {
    ...
    const tex = isMoving ? walkTex : idleTex;
    switchAnimTexture(material, tex, anim);
    advanceAnim(tex, frames, anim, delta);

    sprite.scale.x = PLAYER_SCALE * motion.facingRef.current;
